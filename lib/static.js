...
class Static {
    constructor (client, params) {
        ...
        if (params.ffmpeg) {
            const ffmpegPath = path.resolve(params.ffmpeg);
            if (fs.existsSync(ffmpegPath)) {
                this.m3u8 = new m3u8(ffmpegPath);
            } else {
                console.error("Can't find FFmpeg executable on", ffmpegPath);
            }
        }
            ...
            if (/audio\/ee\//.test(pathname)) {
                if (this.m3u8) {
                    ...
                    return Promise.reject(
                        new Error("This audio URL needs for FFmpeg. Please set ffmpeg path in params")
                    );
                }
            } else {
