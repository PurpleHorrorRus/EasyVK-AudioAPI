...
class Static {
    ...
    parseMore (response, params = {}) {
        switch (typeof response) {
            case "string": {
                const next_from = response.match(/next_from":"(.*?)"/)[1];
            ...
            case "object": {
                params.forcePlaylist = response.playlist
                    && params.forcePlaylist;

                const section_id = params.forcePlaylist
                    ? (response.playlist.id || response.playlist.blockId)
                    : (response.section_id || response.sectionId);

                const next_from = params.forcePlaylist
                    ? response.playlist.nextOffset
                    : (response.next_from || response.nextFrom || response.nextOffset);

                if (!section_id || !next_from) {
                    return null;
                }
    ...
    validateMore(more) {
        return Boolean(more)
            && (more.section_id || more.sectionId)
            && (more.start_from || more.next_from || more.nextFrom || more.nextOffset);
    }
