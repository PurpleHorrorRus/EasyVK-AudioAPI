...
class PlaylistsStatic extends Static {
    ...
    async buildRecommendedPlaylists(parsed, params = {}) {
        ...
        return await Promise.map(items, async item => {
            const playlist = this.getInfo(item);
            ...
            return {
                title: this.unescape(item.querySelector(this.classes.RECOMMENDED_PLAYLIST.title).text),
                background: top.attributes.style.match(backgroundRegex)[1],

                playlist: {
                    owner_id: playlist[0],
                    playlist_id: playlist[1],
                    access_hash: playlist[2],
                    raw_id: `${playlist[0]}_${playlist[1]}`
                },
