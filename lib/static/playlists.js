...
class PlaylistsStatic extends Static {
    ...
    build (html) {
        return HTMLParser.parse(html).querySelectorAll(this.classes.PLAYLIST.ITEM).map(block => {
            const inner = block.innerHTML;
            const title = title_template.test(inner) ? inner.match(title_template)[1] : "";
            const cover = this.parserConfig.cover_regex.test(inner) ? inner.match(this.parserConfig.cover_regex)[1] : "";
    ...
    getGenreByHTML (html) {
        ...
        for (const inner_object of inner) {
            try { 
                ...
                pl_objects = inner_parsed.querySelectorAll(this.classes.PLAYLIST.ITEM);
