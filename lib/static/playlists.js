...
class PlaylistsStatic extends Static {
    ...
    getInfo(block) {
        if (info_template.test(block.innerHTML)) {
            ...
            return { 
                owner_id: Number(match[0]), 
                playlist_id: Number(match[1]), 
        ...
        return await Promise.map(items, async item => {
            ...
            return {
                ...
                owner: {
                    ...
                    avatar: this.fixAvatar(ownerAvatar.attributes.style.match(coversRegex)[1])
                },
    ...
    fixAvatar(avatar) {
        ...
        return ~avatar.indexOf("https://vk.com")
            ? avatar
            : `https://vk.com${avatar}`;
    }
}
