...
class PlaylistsStatic extends Static {
    ...
    builderHTML (html) {
        return !~html.indexOf("RecomsPlaylist")
    ...
    buildRecoms (html) {
        return HTMLParser.parse(html).querySelectorAll(this.classes.RECOMS.BLOCK).map(block => {
            ...
            return { 
                ...
                title: block.querySelector(this.classes.RECOMS.TITLE).text.replaceAll("'", ""),
                subtitle: block.querySelector(this.classes.RECOMS.SUBTITLE).text.replaceAll("'", ""),
                cover: this.getCover(block, this.classes.RECOMS.COVER) 
            };
        });
    }
    ...
    buildCollections (html) {
        const root = HTMLParser.parse(html[0]);
