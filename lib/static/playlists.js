...
class PlaylistsStatic extends Static {
    ...
    build (html) {
        return HTMLParser.parse(html).querySelectorAll(this.classes.PLAYLIST.ITEM.BLOCK).map(block => {
            ...
            const [owner_id, playlist_id, access_hash] = info_template.test(inner) ? inner.match(info_template)[1].split(", ") : [];
            ...
            const subtitle_block = root_block.querySelector(this.classes.PLAYLIST.YEAR);
            
            let year = "";
            let subtitle = "";

            if (subtitle_block) {
                if (subtitle_block.childNodes.length >= 1) {
                    year = subtitle_block.childNodes[0].text;

                    if (subtitle_block.childNodes.length === 3) {
                        subtitle = subtitle_block.childNodes[2].text;
                    }
                }
            }
