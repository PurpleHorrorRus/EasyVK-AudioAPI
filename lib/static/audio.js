...
class AudioStatic extends Static {
    ...
    builderHTML (html) {
        ...
        try {
            map = a_objects.map(a => {
                const json = JSON.parse(a.attributes[this.classes.AUDIO.ATTRIBUTE]);
                return json;  
            });
        } catch (e) {
            map = a_objects.map(a => {
    ...
    getAudioAsObject (audio = []) {
        const source = this.UnmuskTokenAudio(audio[this.AudioObject.AUDIO_ITEM_INDEX_URL], this.user);
    ...
    builder (item) {
        switch (typeof item) {
            case "string" :{
                const raws = this.builderHTML(item);
                return raws.map(a => this.getAudioAsObject(a));
            }
            case "object": {
                return this.getAudioAsObject(item);
            }
        }
    }
