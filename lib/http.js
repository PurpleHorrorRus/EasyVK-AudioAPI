...
class HTTPClient {
    ...
    checkBody (body) {
        if (regex.ACTION_LOGIN.test(body)) {
            return body.match(regex.ACTION_LOGIN)[1] !== "play_rate";
        }

        return regex.ACTION_LOGIN.test(body);
    }
    ...
    async login (params = {}) {
        ...
        configuration.jar = new CookieJar(new FileCookieStore(configuration.cookiesPath));
        const body = await this.requestEndpoint({}, false, false, "");
        if (body && !regex.ACTION_LOGIN.test(body)) {
            return this;
        }
