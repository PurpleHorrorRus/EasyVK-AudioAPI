...
class HTTPClient {
    ...
    async finalRequest (...args) {
        ...
        if (regex.ACTION_LOGIN.test(args[0]) && args[0].match(regex.ACTION_LOGIN)[1] !== "play_rate") {
            await this.login();
    ...
    async login (params = {}) {
        if (!configuration.jar) {
            configuration.jar = configuration.jar || new CookieJar(new FileCookieStore(params.cookies || configuration.cookiesPath));
        }

        const body = await this.requestEndpoint({}, true, false, "feed");
        if (!regex.ACTION_LOGIN.test(body)) {
            setInterval(() => this.login(), 60 * 1000 * 60);
            return this;
        }
