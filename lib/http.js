...
class HTTPClient {
    ...
    async request (url, form = {}, post = true, options = {}) {
        ...
        if (!this.checkBody(body) && redirectCodes.includes(response.status)) {
            ...
            if (options.additionalHeaders) {
                ...
                options.additionalHeaders["Host"] = url.host;
            }
    ...
    async login (params = {}) {
        ...
        const response = await this.request("https://id.vk.com");
        if (!regex.ACTION_LOGIN.test(response)) {
            return this;
        }
        ...
        const matchInit = response.match(/window\.init = (.*?);/)[1];
    ...
    async actLogin () {
        const authParams = {
            ...this.credits,
            auth_token: this.init.auth_token,
            sid: "",
            uuid: this.init.uuid,
            v: this.init.v,
        ...
        const authResponse = await this.request("https://login.vk.com/?act=connect_authorize", authParams, true, {
        ...
        switch (authResponse.type) {
            ...
            case "okay": {
                ...
                return {
                    ...
                    user: authResponse.data.auth_info.user.id
                };
            }
    ...
    async final () {
        await this.request("https://vk.com/feed");
        return this;
    }
