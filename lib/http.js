...
class HTTPClient {
    ...
    async login (params = {}) {
        const valid = await this.validateSession(params.cookies || configuration.cookiesPath);
        ...
        const [, PostLoginURL] = valid.match(regex.ACTION_LOGIN);
    ...
    async validateSession (cookiesPath) {
        ...
        return !regex.ACTION_LOGIN.test(body) ? true : this.clearResponse(body);
    }
