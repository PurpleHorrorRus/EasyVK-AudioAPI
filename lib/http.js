...
const configuration = {
    ...
    endpoints: {
        verify: "https://vk.com/",
        auth: "https://id.vk.com",
        final: "https://vk.com/feed",
        referer: "https://id.vk.com/",
        connectAuthorize: "https://login.vk.com/?act=connect_authorize",
        validatePhone: "https://api.vk.com/method/auth.validatePhone"
    }
};

const regex = {
    ACTION_LOGIN: /al_login\.php/,
...
class HTTPClient {
    ...
    async login (params = {}) {
        ...
        const response = await this.request(configuration.endpoints.verify);
    ...
    async actLogin () {
        const response = await this.request(configuration.endpoints.auth);

        const matchInit = response.match(regex.LOGIN_INIT)[1];
        this.init = JSON.parse(matchInit);
        this.init.v = configuration.apiVersion;
        this.init.uuid = crypto.randomUUID();
        this.init.headers = {
            Origin: configuration.endpoints.auth,
            Referer: configuration.endpoints.referer
        };

        const authResponse = await this.request(configuration.endpoints.connectAuthorize, {
            ...this.credits,
    ...
    async final() {
        await this.request(configuration.endpoints.final);
        return this;
    }

    async sms (lang = "ru") {
        return await this.request(configuration.endpoints.validatePhone, {
            v: this.init.v,
