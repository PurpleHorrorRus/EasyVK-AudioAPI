...
class HTTPClient {
    ...
    async refreshSession () {
        const response = await this.request("https://vk.com/", {}, false);
        const body = this.clearResponse(await response.textConverted());

        if (this.validateJSON(body)) {
            const { location } = this.parseJSON(body);

            if (location && regex.REFRESH_SESSION_REDIRECT_URL.test(location)) {
    ...
    async actLogin (params = {}) {
        let response = await this.request("https://login.vk.com/?act=login", {
            act: "login",
            role: "al_frame",
            expire: "",
            recaptcha: "",
            ...
            headers: {
                ...configuration.headers,
                Cookie: configuration.jar.getCookieStringSync("https://login.vk.com/?act=login"),
                Origin: "https://vk.com",
        ...
        if (regex.ACT_AUTHCHECK.test(body)) {
            response = await this.request("https://vk.com/login?act=authcheck", {}, false);
    ...
    async solveCaptcha (params) {
        const response = await this.request("https://vk.com/al_login.php?act=a_authcheck_code", {
            captcha_sid: params.captcha_sid,
            captcha_key: params.captcha_key,
            ...this.tfa
        });
    ...
    async auth2FA (params) {
        ...
        const response = await this.request("https://vk.com/al_login.php?act=a_authcheck_code", this.tfa);
        const body = await response.textConverted();
        const json = this.parseJSON(body);
