...
class HTTPClient {
    ...
    async request(url, form = {}, post = true, options = {}) {
        ...
        headers.Cookie = configuration.jar.getCookieStringSync(url);
        ...
        if (~[303, 301, 302, 307].indexOf(response.status)) {
            return await this.request(
                response.headers.get("location"),
                response.status === 307 ? options.body : null,
                response.status === 307
            );
        }
    ...
    async actLogin(PostLoginURL, params) {
        ...
        let response = await this.request(PostLoginURL, authParams);
        let body = await response.text();
