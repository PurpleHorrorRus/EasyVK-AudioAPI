...
class HTTPClient {
    ...
    async request (url, form = {}, post = true, options = {}) {
        ...
        if (!cookies) {
            await this.requestEndpoint({}, true, false, "feed");
            return await this.request(...arguments);
        }

        cookies.map(cookie => configuration.jar.setCookieSync(Cookie.parse(cookie), response.url));
    ...
    async finalRequest (...args) {
        args[0] = await args[0].textConverted();

        if (regex.ACTION_LOGIN.test(args[0])) {
            const act = args[0].match(regex.ACTION_LOGIN)[1];
            
            if (act !== "play_rate" && act !== urls.ACT_LOGIN) {
                await this.login();
                return await this.requestEndpoint(...args);
            }
        }

        return this.parseJSON(args[0]);
    }
    ...
    async requestEndpoint (form = {}, post = true, isMobile = false, file = "al_audio.php") {
        ...
        const response = await this.request(url, form, post);
        return await this.finalRequest(response, ...arguments);
    }
    ...
    async login (params = {}) {
        ...
        const body = await this.requestEndpoint({}, false, false, "/");
        ...
        return await this.actLogin({
            to: body.match(regex.LOGIN.TO)[1],
