...
class hls {
    ...
    async downloadM3U8 () {
        ...
        await Promise.map(chunks, async chunk => {
            ...
            if (data) {
                const chunkPath = path.join(this.params.chunks, chunk.name);
            ...
            this.params.onProgress((downloaded / chunks.length) * 100);
        ...
        const result = await new Promise((resolve, reject) => {
            ...
            for (const key of Object.keys(metadata)) {
                ffmpegInstance.outputOptions("-metadata", `${key}=${metadata[key]}`);
            }
