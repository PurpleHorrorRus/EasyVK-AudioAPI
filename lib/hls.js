...
class hls extends events {
    ...
    async downloadCover (audio) {
        if (!audio.coverUrl_p && !audio.album?.thumb) {
            return "";
        }

        const cover = audio.album?.thumb
            ? (audio.album.thumb.photo_1200 || audio.album.thumb.photo_600)
            : audio.coverUrl_p;

        const pathToCover = path.resolve(this.params.chunksFolder, `${audio.id}.jpg`);
    ...
    async downloadM3U8 (audio, outputFolder) {
        let [chunks, cover] = await Promise.all([
            this.parse(audio.url),
            this.downloadCover(audio)
        ]);
    ...
    async downloadMP3 (audio, outputFolder) {
        this.params.name = filenamify(this.params.name);
        const output = path.join(outputFolder, this.params.name + ".mp3");
        const response = await fetch(audio.url, fetchOptions);
     ...
     */
    async download (audio, outputFolder) {
        if (!audio) {
        ...
        const link = audio.url.replace("&long_chunk=1", "");
        ...
        return extension !== "mp3"
            ? await this.downloadM3U8(audio, outputFolder)
            : await this.downloadMP3(audio, outputFolder);
    }
}
